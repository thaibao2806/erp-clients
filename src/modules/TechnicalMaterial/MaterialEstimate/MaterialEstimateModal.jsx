...
import { createMaterialEstimates, updateJMaterialEstimates } from "../../../services/apiTechnicalMaterial/apiMaterialEstimate";
...
const MaterialEstimateModal = ({ open, onCancel, onSubmit, initialValues }) => {
  ...
  const getApprovals = async (refId) => {
    try {
      let res = await getApprovalsByRef(refId, "DTVT");
  ...
  const getApprovalByModulePage = async () => {
    try {
      let res = await getApprovalSetting("TM", "tm-du-tru-vat-tu");
      if (res && res.status === 200) {
  ...
  const getVoucherNo = async () => {
    try {
      let res = await getDocumentNumber("DTVT");
      if (res && res.status === 200) {
      ...
      },
      {
        ...
        render: (_, record) => (
          <Input
            value={record.origin}
            onChange={(e) =>
              handleInputChange(record.key, "origin", e.target.value)
            }
          />
        ),
      },
  ...
  const handleAddApprovals = async (refId, documentNumber) => {
    try {
      ...
      const res = await createApprovals(
        ...
        formattedApprovers,
        documentNumber,
        `/tm/du-tru-vat-tu-chi-tiet/${refId}?type=DTVT`
      );
      if (res && res.status === 200) {
    ...
    if (!initialValues) {
      form.validateFields().then(async (values) => {
        try {
          ...
          let res = await createMaterialEstimates(
            payload.voucherNo,
            payload.voucherDate,
            payload.productName,
            payload.category,
            payload.estimateRound,
            payload.note,
            payload.details
          );
          if (res && res.status === 200) {
      ...
      form.validateFields().then(async (values) => {
        try {
          ...
          let res = await updateJMaterialEstimates(
            initialValues.id,
            payload.voucherNo,
            payload.voucherDate,
            payload.productName,
    ...
    <Modal
      title={
        <span style={{ fontSize: 25, fontWeight: 600 }}>
          {initialValues ? "Cập nhật dự trù vật tư" : "Thêm dự trù vật tư"}
        </span>
      }
      open={open}
      onCancel={() => {
...
export default MaterialEstimateModal;
