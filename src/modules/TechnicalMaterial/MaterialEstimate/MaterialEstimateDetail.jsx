...
const MaterialEstimateDetail = () => {
  ...
  const getApprovals = async () => {
    try {
      let res = await getApprovalsByRef(id, "DTVT");
      if (res && res.status === 200) {
  ...
  const getData = async () => {
    try {
      let res = await getByIDMaterialEstimate(id);
      if (res && res.status === 200) {
  ...
  const handleMenuClick = async ({ key }) => {
    if (key === "edit") {
      ...
      try {
        let res = await deleteMaterialEstimates(data.id);
        if ((res && res.status === 200) || res.status === 204) {
          Modal.success({
            title: "Xóa thành công",
            content: `Đã xóa thành công phiếu`,
          });
          navigator("/tm/du-tru-vat-tu");
        }
      } catch (error) {
        Modal.error({
          title: "Xóa thất bại",
      ...
      <input
        ...
        onChange={async (e) => {
          ...
          for (const file of files) {
            ...
            formData.append("refType", "MaterialEstimate");
...
export default MaterialEstimateDetail;
