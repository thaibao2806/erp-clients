...
const MaterialEstimate = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Link to={`/tm/du-tru-vat-tu-chi-tiet/${record.id}`}>{text}</Link> // ✅ THAY ĐOẠN NÀY
      ),
    },
    {
      title: "Ngày chứng từ",
  ...
  const mobileColumns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <Link
            to={`/tm/du-tru-vat-tu-chi-tiet/${record.id}`}
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          // Gọi API xóa từng ID
          await Promise.all(
            selectedRowKeys.map((id) => deleteMaterialEstimates(id))
          );
    ...
    try {
      ...
      for (const id of selectedRowKeys) {
        ...
        const fileName = matchedItem?.voucherNo
          ? `Dutruvattu_${matchedItem.voucherNo}.xlsx`
          : `Dutruvattu_${id}.xlsx`;
        try {
          let res = await exportExcelME(id);
    ...
    <div style={{ padding: isMobile ? 12 : 16 }}>
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} sm={12} md={8}>
          ...
          <Input
            placeholder="Lần dự trù"
            value={filters.estimateRound}
            onChange={(e) =>
              handleFilterChange("estimateRound", e.target.value)
            }
            size={isMobile ? "small" : "default"}
          />
        </Col>
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              >
                ...
                <Link
                  to={`/tm/du-tru-vat-tu-chi-tiet/${item.id}`}
                  style={{ fontWeight: 600, fontSize: 14 }}
              ...
              <div style={{ fontSize: 12, color: "#666", marginBottom: 2 }}>
                <strong>Hạng mục:</strong> {item.category}
              </div>

              <div style={{ fontSize: 12, color: "#666", marginBottom: 2 }}>
                <strong>Lần dự trù:</strong> {item.estimateRound}
              </div>
