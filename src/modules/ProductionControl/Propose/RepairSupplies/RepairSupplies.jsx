...
import RepairSuppliesModal from "./RepairSuppliesModal";
import { Link } from "react-router-dom";

const { RangePicker } = DatePicker;

const RepairSupplies = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "sochungtu",
      render: (text, record) => (
        <Link to={`/pm/de-xuat/vat-tu-sua-chua-chi-tiet/${record.key}`}>{text}</Link> // ✅ THAY ĐOẠN NÀY
      ),
    },
    {
      title: "Ngày chứng từ",
  ...
  return (
    <div style={{ padding: 5 }}>
      ...
      >
        <h1 style={{ margin: 0 }}>Đề xuất vật tư sửa chữa</h1>
        <Space>
          ...
          <Tooltip title="Xuất excel">
            <Button icon={<FileExcelOutlined />} style={{ background:"#e6f4fb", color:"#0700ad" }}s/>
          </Tooltip>
        </Space>
      </div>
