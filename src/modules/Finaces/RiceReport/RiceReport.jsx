...
const RiceReport = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      fixed: isMobile ? "left" : false,
      render: (text, record) => (
        <Link to={`/fn/nhan-su/bao-com-chi-tiet/${record.id}`}>{text}</Link>
      ),
    },
    {
      title: "Ngày chứng từ",
  ...
  const mobileColumns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <Link
            to={`/fn/nhan-su/bao-com-chi-tiet/${record.id}`}
  ...
  const handleDelete = () => {
    ...
    Modal.confirm({
      title: "Xác nhận xóa",
      content: `Bạn có chắc chắn muốn xóa ${selectedRowKeys.length} báo cáo này không?`,
      okText: "Xóa",
      cancelText: "Hủy",
      onOk: async () => {
        try {
          ...
          Modal.success({
            ...
            content: `${selectedRowKeys.length} báo cáo đã được xóa.`,
          });
        } catch (error) {
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              >
                ...
                <Link
                  to={`/fn/nhan-su/bao-com-chi-tiet/${item.id}`}
